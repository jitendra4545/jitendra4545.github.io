    ...
    return (
        <Box mt='10%' >
            ...
            <SimpleGrid mt='5%' gap="6" columns={{ base: '2', md: '3', lg: '3' }}>
                ...
                {
                    data.map((el) => (
                        <Box   bg="#02054B" m='auto' boxShadow='   #02054B 0px 5px 15px;' justifyContent='center' w={{ base: '11rem', md: "14rem", lg: '15rem' }} p={{ base: '10px 20px', md: '20px 70px' }} h={{ base: '12rem', md: '10rem', lg: '' }} gap='4' >
                            <Image  src={el.imgURL} alt={el.name} />
                            <Text  color='white' fontWeight={'bold'}>{el.name}</Text>
                        </Box>
                    ))
                }
